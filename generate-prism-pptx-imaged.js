// Generate IFC Pitch Deck — Prism-style PPTX (imaged / editable-text variant)
// Output: /Users/jvanwaveren/IFC/IFC_PitchDeck_Prism_imaged.pptx
// Each slide uses a 1920×1080 PNG captured from prism-pitchdeck.html as its
// full-slide background. Text on top is editable.
...
const BG_DIR = "/Users/jvanwaveren/IFC/pitchdeck-backgrounds";
let slideCounter = 0;
function newSlide() {
  slideCounter++;
  const s = pptx.addSlide();
  s.background = { path: `${BG_DIR}/slide-${slideCounter}.png` };
  // All visual shapes are in the background image — neutralize shape-drawing
  // calls so helpers like addGlassPanel / addFlares / per-slide addShape calls
  // become silent noops. Text boxes (addText) still render normally.
  s.addShape = () => s;
  return s;
}
...
function addFlares(_slide) { /* visuals in the background PNG */
}

// Helper: 3D glass panel with bevel + drop shadow
function addGlassPanel(slide, x, y, w, h, opts = {}) {
  ...
  // Top bevel highlight — thin bright line
...
steps.forEach((s, i) => {
  ...
  s4.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
    ...
    rectRadius: 0.12,
    fill: { color: DEEP, transparency: 20 },
    line: { color: GLASS_BORDER, width: 0.5, transparency: 60 },
  });
  s4.addText("OUTPUT", {
...
founders.forEach((f, i) => {
  ...
  s7.addShape(pptx.shapes.OVAL, {
    ...
    fill: { color: f.grad[0], transparency: 20 },
    line: { color: f.grad[1], width: 1.5, transparency: 40 },
  });
...
// GENERATE
// ═══════════════════════════════════════
const outPath = "/Users/jvanwaveren/IFC/IFC_PitchDeck_Prism_imaged.pptx";
