// Generate IFC Business Plan 2026 — Prism-style PPTX (imaged / editable-text variant)
// Output: /Users/jvanwaveren/IFC/IFC_BusinessPlan_2026_imaged.pptx
//
// Same 12 slides as the atmospheric generator, but each slide uses a pre-rendered
// 1920×1080 PNG (captured from ifc-businessplan.html at high fidelity — real CSS
// backdrop-filter blur, radial flares, glass shadows) as its full-slide background.
// All visual style comes from the image. All text sits on top as editable text
// boxes, so you can retype directly in PowerPoint / Keynote / Canva.
//
// Requires: /Users/jvanwaveren/IFC/slide-backgrounds/slide-{1..12}.png
// Regenerate backgrounds with: bash capture-slide-backgrounds.sh
...
// ── Per-slide background wrapper ────────────────────────────
// Replaces pptx.addSlide({ masterName: "PRISM" }) calls so every new slide
// picks up the next numbered background image automatically.
const BG_DIR = "/Users/jvanwaveren/IFC/slide-backgrounds";
let slideCounter = 0;
function newSlide() {
  slideCounter++;
  const s = pptx.addSlide();
  s.background = { path: `${BG_DIR}/slide-${slideCounter}.png` };
  return s;
}

// ── Helpers (no-ops in imaged variant — visuals live in the PNG) ──
function addFlares(_slide) {}
function addGlassPanel(_slide, _x, _y, _w, _h, _opts = {}) {}
function addAccentBar(_slide, _x, _y, _h, _color) {}
function addSpectrumLine(_slide, _x, _y, _w) {}

// (Keep unused function body below as reference; never called.)
function _refSpectrumLine(slide, x, y, w) {
...
// ═══════════════════════════════════════════════════════════
{
  ...
  tags.forEach(t => {
    ...
    s.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
      ...
      line: { color: INK, width: 0.5, transparency: 75 },
    });
...
// ═══════════════════════════════════════════════════════════
{
  ...
  steps.forEach((p, i) => {
    ...
    s.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
      ...
      fill: { color: "FFFFFF", transparency: 35 }, line: { color: "FFFFFF", width: 0.5, transparency: 20 },
    });
...
// ── Write out ───────────────────────────────────────────────
const outPath = "/Users/jvanwaveren/IFC/IFC_BusinessPlan_2026_imaged.pptx";
